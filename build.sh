...
if [[ $1 = index ]] ; then
  ...
  /usr/bin/pandoc -t revealjs -f markdown -s source/index.md \
   --css css/estilo.css -o index.html
  exit 0
fi
...
if [[ $# -eq 1 ]]; then
  ...
  cat source/$1/*.md | \
  /usr/bin/pandoc -t revealjs -f markdown -s - --css ../css/estilo.css  \
   --slide-level=2 -o slides/$1.html
fi
