	...
	var Carousel = {
		init: function(options, elem) {
			...
			self.$li.each(function() {
				if ($(this).outerWidth(false) >= self.maxOuterWidth) {                                             //get max width to apply to all items for smooth movement
					self.maxOuterWidth = $(this).outerWidth(false);
			...
			var margenes = parseFloat(self.$li.css("padding-left")) + parseFloat(self.$li.css("padding-right")); 
			self.$li.css({'width': (self.maxWidth) + 'px'});                                                //give all items uniform width for smooth movement
			self.$wrapper.css({'max-width': ((self.maxWidth * self.options.itemsVisible) -margenes) + 'px', 'width': ((self.maxWidth * self.options.itemsVisible) -margenes) + 'px'});      //adjust wrapper width to show X items
			...
			self.$elem.css("width","999999999px");
			self.events();
		...
		build: function() {
			...
			self.$elem.parent().parent().prepend($('<div />').attr('data-carousel-control', 'left').html('<span></span>')).append($('<div />').attr('data-carousel-control', 'right').html('<span></span>'));
		},
